...
module.exports = {
	...
	rateCourse: async function(studentId, courseId, rate) {
		const course = await User.findById(mongoose.Types.ObjectId(studentId), function(err) {
			if (err) throw Error(err);
		});
		console.log(course);
		for (let i = 0; i < course[0].courses.length; i++) {
			if (course.courses._id === mongoose.Types.ObjectId(courseId)) {
				course.courses.rate = rate;
			}
		}
		await course.save();
	}
