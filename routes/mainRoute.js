...
router.post(
  "/update-info",
  [
    body("Fname", "First Name cannot be empty").isLength({ min: 1 }),
    body("Lname", "Last Name cannot be empty").isLength({ min: 1 }),
    body("phone", "Phone Number must have at least 6 characters long").isLength(
      { min: 6 }
    ),
    body("dob", "Choose a date of birth").custom((value, { req }) => {
